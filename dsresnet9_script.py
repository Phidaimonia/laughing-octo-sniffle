...
x = Reshape([timesteps, mel_bins, 1])(x)

x = Conv2D(embed_dim, 3, strides=1, activation="ReLU", padding="same")(x)
x = AveragePooling2D([4, 3])(x)

x = InvertedResidual(embed_dim, 1, expansion_factor=3)(x)
x = InvertedResidual(embed_dim, 1, expansion_factor=3)(x)
x = InvertedResidual(embed_dim, 1, expansion_factor=3)(x)
x = InvertedResidual(embed_dim, 1, expansion_factor=3)(x)

x = Conv2D(embed_dim, 3, activation="ReLU", padding="same")(x)
